            ...
            destinationLocations.forEach((destination, idx) => {
              try {
                ...
                if (geocodedDestination) {
                  // Use a fixed departure time of 10:00 AM PST (Pacific Standard Time)
                  const fixedDepartureTime = new Date();
                  // Set to today's date
                  fixedDepartureTime.setHours(10, 0, 0, 0); // 10:00:00.000 local time
                  
                  // Add a day if it's already past 10:00 AM
                  if (new Date() > fixedDepartureTime) {
                    fixedDepartureTime.setDate(fixedDepartureTime.getDate() + 1);
                  }
                  
                  // PST is UTC-8 (or UTC-7 during daylight saving time)
                  // We'll use toISOString() which will convert to UTC automatically
