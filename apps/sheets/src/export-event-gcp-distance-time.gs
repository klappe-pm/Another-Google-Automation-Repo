/**
 * Script Name: export-event-gcp-distance-time
 * 
 * Script Summary:
...
function exportAllCalendarEvents() { // Enable Advanced Calendar Service // Go to Resources > Advanced Google Services and enable Calendar API // Define the three destination locations
  ...
  var timeMax = Utilities.formatDate(tomorrow, "UTC", "yyyy - MM - dd'T'HH:mm:ss'Z'"); // Retrieve all accessible calendars;
  var calendars = CalendarApp.getAllCalendars(); // Prepare data array with headers;
  var headers = [;
  ...
  var data = [headers]; // Helper functions;
  /**
 ...
 * @returns {any} The result
  ...
  calendars.forEach(function (calendar) {
    ...
    var calName = calendar.getName(); // Retrieve events for the calendar;
    var options = {
      timeMin: timeMin,
      timeMax: timeMax,
      singleEvents: true,
    ...
    var events = response.items; // Process each event;
    events.forEach(function (event) {
      try {
        ...
        var endTime = event.end.dateTime ? new Date(event.end.dateTime) : ""; // Calculate duration;
        var duration;
        ...
        var endTimeStr = endTime ? Utilities.formatDate(endTime, "UTC", "HH:mm") : ""; // Create hyperlink for location;
        var locationHyperlink = eventLocation ? `=HYPERLINK("https: // www.google.com / maps / search / ?api=1&query=${encodeURIComponent(eventLocation)}", "${eventLocation}")` : ""; // Get additional date fields;
        ...
        var dayOfWeek = getDayOfWeek(eventDate); // Calculate outbound distances and times if location is available;
