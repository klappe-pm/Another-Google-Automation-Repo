...
const categoryContent = await app.vault.read(categoryFile);
...
const subCategoryContent = await app.vault.read(subCategoryFile);
...
// Set the content with frontmatter, line breaks, and H1 header
tR = `---
...
// IMPORTANT: Rename the file after creation
// We need to wait until all templates have executed to rename
