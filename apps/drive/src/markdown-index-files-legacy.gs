...
function listFilesInFolder() {
  ...
  var properties = PropertiesService.getScriptProperties(); // Create or get the sheet with date stamp;
  ...
  var sheet = spreadsheet.getSheetByName(newSheetName); // Create new sheet if it doesn't exist for today;
  ...
  var processedIds = JSON.parse(properties.getProperty('processedIds') || '[]'); // Determine the top - level folder (either specified folder or root);
  var topFolder = TOP_LEVEL_FOLDER_ID ? DriveApp.getFolderById(TOP_LEVEL_FOLDER_ID) : DriveApp.getRootFolder(); // Process the top - level folder recursively;
...
function processFolder(folder, pathArray, depth, sheet, processedIds) {
  ...
  var fileData = [];
  var folderId = folder.getId(); // Process all files in the current folder;
  while (files.hasNext()) {
    var file = files.next();
    var fileId = file.getId(); // Skip if file was already processed in a previous run;
    if (processedIds.includes(fileId)) {
      continue; // Skip already processed files;
    }

    var fileName = file.getName(); // Skip Markdown files (customization point);
    if (fileName.toLowerCase().endsWith('.md')) {
      continue;
    } // Gather file metadata
    var fileUrl = file.getUrl();
    var createdDate = Utilities.formatDate(file.getDateCreated(), Session.getScriptTimeZone(), 'yyyy - MM - dd');
    var lastUpdated = Utilities.formatDate(file.getLastUpdated(), Session.getScriptTimeZone(), 'yyyy - MM - dd'); // Create hyperlink formula for clickable file name;
    var hyperlinkFormula = '=HYPERLINK("' + fileUrl + '","' + fileName + '")'; // Create row with folder hierarchy first, then data attributes, then IDs;
    var row = new Array(11).fill(''); // Initialize with empty strings for all columns // Fill in folder hierarchy (columns 1 - 6);
  ...
  while (subfolders.hasNext() && depth < 5) { // Limit depth to 5 levels;
    ...
    var newPath = pathArray.concat([subfolder.getName()]);
