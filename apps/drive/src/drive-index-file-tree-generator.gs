...
/**
 * Main function to initiate the file tree generation process.
 * Prompts the user for the root folder ID and starts the recursive processing.
 */
...
/**
 * Generates the file tree structure in the active sheet starting from the given root folder.
 * @param {Folder} rootFolder The Google Drive Folder object to start from.
 */
function generateTree(rootFolder) {
  ...
  // Array to hold all row data (as RichTextValue objects)
  const outputData = [];
  let maxColumns = 1; // Start with 1 for the full path column

  // --- Start Recursive Processing ---
  // The path array stores FOLDER objects
  processFolder(rootFolder, [rootFolder], outputData, timezone, dateFormat);
  // ----------------------------------
  ...
  // Path Col + Max Folder Cols + File Col + Created Col + Updated Col
  outputData.forEach(row => {
    // The number of folder links is row.length - 4 (Full Path, File Link, Created, Updated)
    // Total columns needed for this row is (row.length - 4) + 1 (Full Path) + 1 (File) + 1 (Created) + 1 (Updated) = row.length
  ...
  // --- Create Header Row ---
  const headerRow = [SpreadsheetApp.newRichTextValue().setText("Full Path").build()];
  // Calculate number of folder columns needed
  ...
  headerRow.push(SpreadsheetApp.newRichTextValue().setText("Date Updated").build());

  // --- Prepare Data for Writing ---
  // Ensure all rows have the same number of columns (padding with empty values if needed)
  // and convert all simple values to RichTextValue
  const finalOutput = [headerRow]; // Start with the header
  outputData.forEach(rowData => {
    const paddedRow = [];
    let currentColumnIndex = 0;

    // 1. Full Path (already RichTextValue)
    paddedRow.push(rowData[currentColumnIndex++]);

    // 2. Folder Links (already RichTextValue) - calculate how many there are
    ...
    // 4. Created Date (already RichTextValue)
    paddedRow.push(rowData[currentColumnIndex++]);

    // 5. Updated Date (already RichTextValue)
    paddedRow.push(rowData[currentColumnIndex++]);
  ...
  // --- Write Data to Sheet ---
  if (finalOutput.length > 1) { // Check if there's data beyond the header
    sheet.getRange(1, 1, finalOutput.length, maxColumns)
         .setRichTextValues(finalOutput); // Use setRichTextValues for hyperlinks

    // --- Formatting (Optional) ---
    // Autofit columns for better readability (can be slow on very large datasets)
     ...
     sheet.getRange(2, maxColumns - 1, sheet.getLastRow() -1 , 2).setNumberFormat(dateFormat);
     // Freeze Header row
     sheet.setFrozenRows(1);

  } else {
     // Only write header if no files were found but processing happened
...
/**
 * Recursively processes a folder, adding file information to the output array,
 * and then calling itself for subfolders.
 *
 * @param {GoogleAppsScript.Drive.Folder} folder The current folder to process.
 * @param {Array<GoogleAppsScript.Drive.Folder>} pathArray An array of FOLDER objects representing the path from the root to the current 'folder'.
 * @param {Array<Array<Object>>} outputData The main array collecting row data (as RichTextValue objects).
 * @param {string} timezone The timezone string for date formatting.
...
function processFolder(folder, pathArray, outputData, timezone, dateFormat) {
  ...
  const fullPathPrefix = pathArray.map(f => f.getName()).join(' > '); // Build path string
  ...
  while (files.hasNext()) {
    ...
    // Column A: Full Path (String -> RichTextValue)
    rowData.push(SpreadsheetApp.newRichTextValue().setText(`${fullPathPrefix} > ${fileName}`).build());

    // Columns B, C, ...: Folder Links (RichTextValue)
    ...
    rowData.push(SpreadsheetApp.newRichTextValue().setText(createdDate).build());

    // Next Column: Updated Date (String -> RichTextValue)
    rowData.push(SpreadsheetApp.newRichTextValue().setText(updatedDate).build());
  ...
  while (subFolders.hasNext()) {
    ...
    // IMPORTANT: Create a *new* path array for the recursive call
    const newPathArray = pathArray.concat([subFolder]);
    // Recursive call
