...
function findAndPrintDocs() {
  try { // 1. Prompt for the owner's email addresses
    var ownerEmails = promptForOwnerEmail(); // 2. Check if the Drive API is authorized;
    ...
    var sharedFiles = searchSharedFiles(); // 4. Combine and filter results;
    var filteredFiles = filterFilesByOwner(ownedFiles.files, sharedFiles.files, ownerEmails); // 5. Create or get the output spreadsheet;
    var sheet = createOrGetOutputSheet(ownerEmails); // 6. Check if any files were found after filtering;
...
function promptForOwnerEmail() {
  var ownerEmailsInput = Browser.inputBox("Enter the owner's email addresses (comma - separated):"); // Validate and split email input;
  if (!ownerEmailsInput) {
