...
function exportComments() {
  try { // Get document ID from user input
    var docId = Browser.inputBox("Enter the document ID:"); // Get folder ID from user input;
    var folderId = Browser.inputBox("Enter the folder ID to save the spreadsheet:"); // Get comments from the document (fetch only the IDs initially);
    var comments = Drive.Comments.list(docId, { fields: 'items(id)' }); // Prepare data for the spreadsheet;
    var sheetData = [];
    if (comments.items && comments.items.length > 0) {
      for (var i = 0; i < comments.items.length; i ++ ) {
        var commentId = comments.items[i].id; // Get the full comment object using get();
        var comment = Drive.Comments.get(docId, commentId); // Process the original comment;
        ...
        if (comment.replies && comment.replies.length > 0) {
          for (var j = 0; j < comment.replies.length; j ++ ) {
            var replyId = comment.replies[j].id; // Get the full reply object using get();
    ...
    var sheet = newSpreadsheet.getSheets()[0]; // Set headers;
...
function processComment(comment) {
  var createdDate = new Date(comment.createdDate); // We can't calculate elapsed time here anymore since we fetch replies separately;
  return [;
