...
function createEmailCountSpreadsheet() { // Calculate the date 180 days ago
  ...
  var threads = GmailApp.search('after:' + cutoffDate.toISOString().split('T')[0]); // Create an object to store email counts;
  var emailCounts = {}; // Iterate through threads and count emails;
  ...
  var sheet = spreadsheet.getActiveSheet(); // Set headers;
