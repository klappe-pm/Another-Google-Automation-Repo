...
function deduplicateRideEmails() { // Get the "RideReceipts" sheet
  ...
  var clusterColumn = headers.indexOf('cluster');
  var dupThreadColumn = headers.indexOf('duplicateThreadId');
  var emailListColumn = headers.indexOf('emailIdList'); // Add dedupeGmail column if it doesn't exist;
  if (dedupeColumn === - 1) {
  ...
  for (var i = 1; i < data.length; i ++ ) {
    ...
    var key = threadId + '|' + tripStartTime; // Unique key;
    if (!groupMap[key]) {
      groupMap[key] = [];
    }
    groupMap[key].push({ row: data[i], index: i + 1, threadId: threadId });
  } // Track cluster words and email IDs per Thread ID
  var threadClusterWords = {};
  var threadEmailLists = {}; // Process each group (Thread ID + Trip Start Time);
  for (var key in groupMap) {
    var rows = groupMap[key];
    var earliestDate = null;
    var highestTip = - Infinity;
    var bestRowIndex = null;
    var threadId = rows[0].threadId;
    var duplicateEmailIds = []; // Find earliest DateReceived and highest TipCost, collect duplicate Message IDs;
    ...
    for (var j = 0; j < rows.length; j ++ ) {
      ...
      var dupThreadMark = isKeep ? '' : threadId;
      var emailListMark = (isKeep || !hasDuplicates) ? '' : threadEmailLists[key];
