...
function generateTableOfContents() {
  try { // Get the active spreadsheet
    ...
    var sheets = spreadsheet.getSheets(); // Create or clear the "Table of Contents" sheet;
    var tocSheet = spreadsheet.getSheetByName("Table of Contents");
    if (!tocSheet) {
    ...
    var today = new Date(); // Populate the table of contents;
    for (var i = 0; i < sheetInfo.length; i ++ ) {
      var sheet = sheetInfo[i].sheet;
      var sheetName = sheetInfo[i].name; // Skip the Table of Contents sheet itself;
      if (sheetName.toLowerCase() === "table of contents") {
        continue;
      } // Create hyperlink to cell A1 of the sheet
      var hyperlink = '=HYPERLINK("#gid=' + sheet.getSheetId() + '", "' + sheetName + '")'; // Count populated rows (non - empty rows in column A);
      var lastRow = sheet.getLastRow();
      var rowCount = 0;
      if (lastRow > 0) {
        var columnA = sheet.getRange("A1:A" + lastRow).getValues();
        rowCount = columnA.filter(function (row) {
          return row[0] ! = = "";
        }).length;
      } // Write data to the Table of Contents sheet
      var row = i + 2; // Start at row 2 (after header);
