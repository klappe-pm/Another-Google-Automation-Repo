...
function listGmailLabelsToSheet() {
  try { // Format dates for Gmail search
    ...
    var spreadsheet = SpreadsheetApp.getActiveSpreadsheet(); // Get or create 'Labels' sheet;
    ...
    var labelData = labels.map(function (label) {
      var labelName = label.getName(); // Build search query with date range;
      var baseQuery = `label:${labelName} after:${formattedStartDate} before:${formattedEndDate}`; // Count total threads with this label in date range;
      var totalThreads = GmailApp.search(baseQuery).length; // Count unread threads with this label in date range;
