...
function listEmails() { // Get the current date and the date 7 days ago.;
  var today = new Date();
  var startDate = new Date(today.getTime() - 7 * 24 * 60 * 60 * 1000); // Log the start and end dates for email retrieval.;
  Logger.log('Retrieving emails from: ' + startDate + ' to: ' + today); // Get the Gmail threads.;
  var threads = GmailApp.search('after:' + startDate.toISOString().slice(0, 10)); // Get the week number.;
  var weekNumber = getWeekNumber(today); // Get the spreadsheet with the specified file ID.;
  var ss = SpreadsheetApp.openById('1_9j6gUp - lIIWLQdVGDKWWuwntKvT4m9eIpW3dsDe1kc'); // Create a new sheet tab named in 'yyyy - WW' format.;
  var sheetName = today.getFullYear() + ' - ' + weekNumber;
  var sheet = ss.getSheetByName(sheetName) || ss.insertSheet(sheetName); // Add headers to the sheet.;
  ...
  var emailData = []; // Iterate through the threads and extract email data.;
  for (var i = 0; i < threads.length; i ++ ) {
    var thread = threads[i];
    var messages = thread.getMessages(); // Log the thread being processed.;
    ...
    for (var j = 0; j < messages.length; j ++ ) {
      ...
      var subject = message.getSubject(); // Get the read / unread status using the thread.;
      var isRead = thread.isUnread() ? "Unread" : "Read"; // Log the email being processed.;
