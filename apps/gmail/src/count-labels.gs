...
function countEmailsWithAndWithoutLabels() { // Set the start date;
  ...
  var query = `after:${Utilities.formatDate(startDate, Session.getScriptTimeZone(), 'yyyy / MM / dd')} - in:spam - in:trash`; // Search for threads matching the query;
  var threads = GmailApp.search(query); // Initialize counters;
  var emailsWithLabel = 0;
  var emailsWithoutLabel = 0; // Process each thread;
