// Summary: 
// This Google Apps Script is designed to export emails from Gmail to PDF format and store them in Google Drive.
// It also logs relevant email data into a Google Sheet. The script utilizes the GmailUtils library to handle
// email processing, including converting emails to HTML and PDF, embedding images, and handling attachments.
// Successfully using this script enables users to automate the process of archiving emails as PDFs and maintaining
// a structured log of email data in a Google Sheet.

// Functions:
// - GmailUtils.eachMessage: Iterates over Gmail messages matching a query.
// - GmailUtils.messageToPdf: Converts Gmail messages to PDF format.
// - GmailUtils.messageToHtml: Converts Gmail messages to HTML format.
// - GmailUtils.formatDate: Formats the date of a Gmail message.
// - GmailUtils.emailGetAvatar: Fetches the avatar image for an email address.
// - GmailUtils.embedHtmlImages: Embeds remote images in HTML content.
// - GmailUtils.embedInlineImages: Embeds inline images in HTML content.
// - GmailUtils.renderDataUri: Converts an image to a data URI.
// - GmailUtils.fetchRemoteFile: Fetches a remote file.
// - GmailUtils.isValidUrl: Validates a URL.
// - GmailUtils.sanitizeFilename: Sanitizes a filename.
// - GmailUtils.formatEmails: Formats email addresses in HTML.
// - GmailUtils.isa: Checks if an object is of a specific class.
// - GmailUtils.md5: Computes the MD5 hash of a string.
// - exportEmailsToPDF: Main function to export emails as PDFs.
// - getOrCreateFolder: Helper function to get or create a folder in Google Drive.
// - insertDataIntoSheet: Helper function to insert data into a Google Sheet.

// Dependencies:
// - GmailApp: For interacting with Gmail.
// - DriveApp: For interacting with Google Drive.
// - SpreadsheetApp: For interacting with Google Sheets.
// - UrlFetchApp: For fetching remote files.
// - Utilities: For various utility functions.
// - Session: For getting the script time zone.

// Inputs that can be changed to alter the results:
// - labelName: The Gmail label or query to search for emails.
// - rootFolderName: The name of the root folder in Google Drive.
// - pdfFolderName: The name of the folder to store PDFs.
// - spreadsheetId: The ID of the Google Sheet to log email data.
// - sheetName: The name of the sheet within the Google Sheet.

// Import GmailUtils library
const GmailUtils = {
  ...
  emailGetAvatar: function(email) {
    ...
    if (!avatar && ['gmail', 'hotmail', 'yahoo'].every((s) => domain.indexOf(s) === -1)) {
      avatar = this.fetchRemoteFile(`http://${domain}/apple-touch-icon.png`) ||
               this.fetchRemoteFile(`http://${domain}/apple-touch-icon-precomposed.png`);
    }
    return avatar;
  },
...
function exportEmailsToPDF() {
  try {
    ...
    const sheetName = "Inbox";

    // Step 1: Fetch threads with the label
    const threads = GmailApp.search(`label:${labelName}`);
    if (threads.length === 0) {
      Logger.log(`No emails found with the label "${labelName}".`);
      return;
    }
    Logger.log(`Found ${threads.length} thread(s) with the label "${labelName}".`);

    // Step 2: Get or create the root folder
    ...
    const docsToConvertFolder = getOrCreateFolder(docsToConvertFolderName, rootFolder);
    ...
    GmailUtils.eachMessage(`label:${labelName}`, (message) => {
     ...
     // const sender = extractDisplayNameOrEmail(fromField);
     // const senderAlias = fromField.match(/<(.*)>/);
     // const senderAliasValue = senderAlias ? senderAlias[1] : "Unknown Alias";
      ...
      // Save the Google Doc in the "Docs to Convert" folder
      ...
      const pdfLink = labelFolder.createFile(pdfFile.setName(`${docTitle}.pdf`)).getUrl();

     // Updated Gmail link generation
...
function insertDataIntoSheet(emailData, spreadsheetId, sheetName) {
  ...
  // Add checkboxes to the "Convert MD", "To Reply", and "Processed" columns
